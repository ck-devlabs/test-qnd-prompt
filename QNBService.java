...
public class DocumentIngestionJob implements Job {
     ...
     * <p>
     * Retrieves {@link RequestScopedJobParams}, the data extraction unique ID, and the
     * resolved {@link PromptModel} from the {@link JobDataMap}; resolves the appropriate
     * {@link IngestionStrategy} based on source type; ingests documents; updates
     * request_state with the resulting document count; and schedules the data
     * extraction job(s) for the ingested documents.
     *
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        PromptModel promptModel =
                (PromptModel) dataMap.get("promptModel");
        ...
        try {
            ...
            List<ExtractionJobMap> extractionJobMaps =
                    buildExtractionJobMaps(jobParams, serverRequestUID, supportedDocumentsToExtract);
     ...
     * used to uniquely identify the extraction job/trigger for that document.
     *
     * @param jobParams                   the original request-scoped job params
     * @param serverRequestUID            the unique ID for this extraction run
    ...
    private List<ExtractionJobMap> buildExtractionJobMaps(
        ...
        return supportedDocumentsToExtract.stream()
                .map(storedFileInfo -> {
                    ...
                    jobMap.put("jobParams", jobParams);
                    jobMap.put("dataExtractionUniqueID", serverRequestUID);
