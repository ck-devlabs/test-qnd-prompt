...
import com.hanover.entp.dataext.model.api.aiextraction.AIExtractionRequestData;
import com.hanover.entp.dataext.model.api.aiextraction.AIExtractionRequestSource;
import com.hanover.entp.dataext.model.api.RequestHeader;
...
public class DocumentIngestionJob implements Job {
     ...
     * <p>
     * NOTE: storageType is not currently captured on RequestScopedJobParams (it's
     * commented out in its constructor) - add it there (and to AppConstants.QUARTZ_*
     * keys / getJobDataMap()) so it can be populated below instead of left blank.
     * Also, requestTime is not carried on RequestScopedJobParams; using current
     * time as a placeholder since the original request time isn't preserved.
     *
    ...
    private AIExtractionRequest buildExtractionRequest(RequestScopedJobParams jobParams) {
        ...
        RequestHeader reqHeader = RequestHeader.builder()
                ...
                .requestTime(Instant.now().toString()) // original requestTime not preserved on jobParams
                .build();

        AIExtractionRequestSource documentSource = AIExtractionRequestSource.builder()
                .storageType(jobParams.getStorageType()) // TODO: add storageType to RequestScopedJobParams
                .storageLocation(jobParams.getStorageURL())
                .build();

        AIExtractionRequestData data = AIExtractionRequestData.builder()
